  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: 24 }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--ios-label3)', marginBottom: 24 }}>API connections, risk limits and preferences.</p>

        <div style={{ padding: 20, borderRadius: 16, background: 'var(--ios-bg2)', boxShadow: 'var(--ios-shadow)', marginBottom: 12 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 20 }}>API Connections</h3>
          {[
            { label: 'Binance API Key', value: binanceKey, set: setBinanceKey, placeholder: 'Enter Binance API key...', status: 'Not connected' },
            { label: 'Polymarket CLOB Key', value: polymarketKey, set: setPolymarketKey, placeholder: 'Enter Polymarket CLOB key...', status: 'Not connected' },
            { label: 'CoinGecko API Key', value: coingeckoKey, set: setCoingeckoKey, placeholder: 'Enter CoinGecko key (free tier)...', status: 'Not connected' },
          ].map(api => (
            <div key={api.label} style={{ marginBottom: 16 }}>
              ...
              <input type="password" value={api.value} onChange={e => api.set(e.target.value)} placeholder={api.placeholder} style={{
                ...
                fontSize: 14, background: 'var(--ios-bg)', color: 'var(--ios-label)',
              }} />
            </div>
          ))}
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {saved && <span style={{ fontSize: 13, color: 'var(--ios-green)' }}>Saved</span>}
          </div>
        </div>

        <div style={{ padding: 20, borderRadius: 16, background: 'var(--ios-bg2)', boxShadow: 'var(--ios-shadow)', marginBottom: 12 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16 }}>Global Risk Limits</h3>
          {[
        ...
        <div style={{ padding: 20, borderRadius: 16, background: 'var(--ios-bg2)', boxShadow: 'var(--ios-shadow)', marginBottom: 12 }}>
          ...
          {[
            ...
            <div key={n.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '0.5px solid var(--ios-bg)' }}>
              ...
              <div style={{ width: 44, height: 26, borderRadius: 13, background: n.on ? 'var(--ios-green)' : 'var(--ios-separator)', position: 'relative' }}>
                <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#fff', position: 'absolute', top: 2, left: n.on ? 20 : 2, boxShadow: '0 1px 3px rgba(0,0,0,0.15)' }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: 20, borderRadius: 16, background: 'var(--ios-bg2)', boxShadow: 'var(--ios-shadow)' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {(['light', 'dark'] as const).map(t => (
              <button key={t} onClick={() => { if (t !== theme) toggleTheme(); }} style={{
